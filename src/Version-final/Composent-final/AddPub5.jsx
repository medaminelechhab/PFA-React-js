...
const Add_Pub5 = () => {
    ...
    const { image, besoin, services, budget, titre, setimage, json } = useContext(AnnonceContext)
    ...
    const handleImageChange = (e) => {
        ...
        const imagesArray = selectedImages.map((image) => ({
            file: image,
    ...
    const handleButtonClick2 = async () => {
        console.log('JSON à envoyer : ', json); // Affichez le JSON que vous envoyez pour le débogage
        const json2 = {
            "titre": titre,
            "besoin": besoin,
            "services": services,
        ...
        try {
            const response = await apiService('http://localhost:8089/annonces', 'POST', json2);

            console.log('Réponse du serveur : ', response);

            // Vous pouvez effectuer d'autres actions après avoir reçu la réponse du serveur si nécessaire
        } catch (error) {
