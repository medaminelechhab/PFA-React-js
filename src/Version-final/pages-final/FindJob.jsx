...
export default function FindJob() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch("http://localhost:8089/annonces");
                if (!response.ok) {
                    throw new Error(`HTTP error! Status: ${response.status}`);
                }
                const jsonData = await response.json();
                setdata(jsonData);
                console.log(jsonData); // Afficher les données récupérées dans la console
            } catch (error) {
        ...
        fetchData(); // Appeler la fonction fetchData au montage du composant
    }, []); // Le tableau vide [] signifie que ce useEffect s'exécute une seule fois après le montage initial du composant
            ...
            <div className="flex">
                <div className="pt-10 max-w-60 ml-48">
                    ...
                    <div>
                        ...
                        <div className="text-justify pt-2 font-sans">
                            Postez votre projet gratuitement pour être mis en relation avec les nombreux freelances disponibles.
                        </div>
                        <button className="bg-green-600 px-4 py-3 text-white rounded-full mt-4 text-lg">
                            Trouve un freelance
                        </button>
                    </div>
                </div>
                <div className="ml-14 mr-48">
                    ...
                    <div className="mt-3 overflow-y-auto max-h-[2000px] custom-scrollbar">
                        {/* Mapping des ProjectCard avec les données */}
